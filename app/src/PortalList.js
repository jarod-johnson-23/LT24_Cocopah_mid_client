...
function PortalList() {
  ...
  const [portalListFile, setPortalListFile] = useState(null);
  ...
  const [specialOffers, setSpecialOffers] = useState(
    Array(5).fill({
      ...
      type: "Tier-based",
      qualifying_text: "",
      tierData: Array(20).fill({ yesNo: "", amount: "", time: "", text: "" }),
      cardData: { T1: "", T2: "", T3: "" }, 
    })
  );
  ...
  const onPortalListFileUpload = (extractedColumns, uploadedFile) => {
    setPortalListFile(uploadedFile);
  };
        ...
        i === index
          ? {
              ...
              tierData: !offer.visible
                ? Array(20).fill({ yesNo: "", amount: "", time: "", text: "" })
                : offer.tierData,
              cardData: !offer.visible
                ? {
                    T1: { yesNo: "", amount: "", time: "", text: "" },
                    T2: { yesNo: "", amount: "", time: "", text: "" },
                    T3: { yesNo: "", amount: "", time: "", text: "" },
                  }
                : offer.cardData,
            }
          : offer
  ...
  const submitFiles = async () => {
    try {
      ...
      if (portalListFile) formData.append("portalListFile", portalListFile);
      ...
      const response = await axios.post(
        `${API_BASE_URL}/generate-portal-list`,
        formData,
        {
          headers: {
      ...
      console.log("Portal list generation successful");
      setIsLoading(false);
      alert("Portal list generation successful!");
    } catch (error) {
      console.error("Error generating portal list:", error);
      setIsLoading(false);
      alert("Error generating portal list. Please try again.");
    }
  };
    ...
    <>
      ...
      <div className="ml-file-drops">
        ...
        <div className="file-header-combo">
          <h4>Portal List</h4>
          <FileDropComponent
            onFileUpload={onPortalListFileUpload}
            className="file-area"
          />
        </div>
        <div className="file-header-combo">
          <h2>Generate Portal List</h2>
          <p>
            ...
            {offer.visible && (
              <>
                ...
                <div className="cocopah-text-input">
                  <input
                    type="text"
                    value={offer.qualifying_text}
                    onChange={(e) =>
                      handleQualifyingTextChange(index, e.target.value)
                    }
                  />
                </div>
                    ...
                    <thead>
                      <tr>
                        ...
                        <th>
                          ...
                          <button
                            onClick={() =>
                              handleCopyDown(index, "tierData", "text")
                            }
                          >
                            Copy Down
                          </button>
                        ...
                        (tier, rowIndex) => (
                          <tr key={rowIndex}>
                            ...
                            <td>
                              <input
                                type="text"
                                value={offer.tierData[rowIndex]?.text || ""}
                                onChange={(e) =>
                                  handleTierDataChange(
                                    index,
                                    rowIndex,
                          ...
                          (cardType, rowIndex) => (
                            <tr key={rowIndex}>
                              ...
                              <td>
                                <input
                                  type="text"
                                  value={offer.cardData[cardType]?.text || ""}
                                  onChange={(e) =>
                                    handleCardDataChange(
                                      index,
                                      cardType,
